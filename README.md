...
blender-axi scene --json
blender-axi start
blender-axi stop
blender-axi setup hooks
```

`exec` wraps Python before sending it, so failures against the unmodified addon include the full traceback and stdout printed before failure. `build` performs build, save, glTF export, and camera rendering in one socket round-trip, with save before export and render.
