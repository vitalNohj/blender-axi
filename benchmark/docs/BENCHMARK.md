...
6. Stage every pinned toolchain the arms invoke outside `/Users/`, using the exact revisions in `benchmark/config/frozen.json`, and prove the staged files are byte-identical to artifacts built from those revisions before live execution. The transcript policy denies any tool event containing `/Users/`, and an agent that resolves a binary with `command -v` embeds its absolute path in later calls. Putting the directory on `PATH` is not sufficient. Each cell also runs under a fresh empty `HOME` and does not inherit caller `UV_CACHE_DIR` or `UV_PYTHON_INSTALL_DIR`, so any staged launcher that discovers an interpreter or cache relative to those values must carry fixed pins in its arm configuration. The BlenderMCP arm is launched through `uvx`, which resolves a Python interpreter at spawn time: warm cache and Python-install directories outside `/Users/`, pre-install a managed interpreter satisfying the server `requires-python` floor (blender-mcp 1.6.5 needs >= 3.10), set the two fixed `UV_*` paths in the staged arm, and pass an explicit `--python <version>` alongside `--offline`. Without the explicit pin, `uvx` falls back to the system `/usr/bin/python3` (3.9.6 on macOS), resolution fails, and the arm dies as an `initialize timed out` extension-load error scored as an infrastructure-invalid cell.
...
The runner checks the invalid-attempt and critical-failure ceilings before starting another cell and enforces the campaign wall limit as a hard deadline across active agent execution and grading. Deadline expiry aborts the active processes, cleans up owned process groups, and returns `wall_time_ceiling` without recording a partial attempt. There is no dollar ceiling because cost is excluded; the campaign wall limit is the binding brake. Campaign execution is sequential. A cross-arm policy violation invalidates the attempt and contributes to the invalid ceiling.
