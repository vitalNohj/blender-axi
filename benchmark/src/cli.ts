	...
	for (const name of parsed.flags.keys())
		if (!valid.has(name))
			throw new UsageError(
				`unknown flag ${name}; valid flags: ${[...valid].join(", ")}`,
			);
}
...
function listFlag(parsed: Parsed, name: string): string[] | undefined {
	...
	if (typeof value !== "string")
		throw new UsageError(`${name} requires comma-separated values`);
	return value
		.split(",")
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
	...
	const benchmarkRoot = resolve(
		stringFlag(parsed, "--benchmark-root", DEFAULT_ROOT),
	);

	if (command === "fixtures") {
		allowed(parsed, ["--benchmark-root", "--blender", "--force"]);
		if (subcommand === "create") {
			const blender = stringFlag(
				parsed,
				"--blender",
				process.env.BLENDER_EXECUTABLE ??
					"/Applications/Blender.app/Contents/MacOS/Blender",
			);
			const index = await createFixtures(benchmarkRoot, blender, {
				force: boolFlag(parsed, "--force"),
			});
			print({
				ok: true,
				fixtures: index.entries.length,
		...
		if (subcommand === "verify") {
			...
			print({
				ok: result.ok,
				fixtures: result.index.entries.length,
	...
	if (command === "plan") {
		...
		if (subcommand === "preflight")
			plan = await generatePreflightPlan(benchmarkRoot, seed);
		else if (subcommand === "pilot")
			plan = await generatePlan(benchmarkRoot, {
				kind: "pilot",
				taskIds: ["P1", "P2", "P3", "P4", "P5", "P6"],
				replicates: numberFlag(parsed, "--replicates", 5),
				seed,
			});
		else if (subcommand === "selected")
			plan = await generatePlan(benchmarkRoot, {
				kind: "selected",
				taskIds: listFlag(parsed, "--tasks") ?? ["P1"],
				replicates: numberFlag(parsed, "--replicates", 1),
				seed,
			});
		...
		print({
			...
			sequences: plan.cells
				.filter((cell) => cell.order_in_pair === 1)
				.map((cell) => `${cell.task_id}:${cell.sequence}`),
		});
		return 0;
	}
	...
	if (command === "preflight") {
		...
		if (!live && subcommand !== "check")
			throw new UsageError("preflight requires check or execute");
		const checks = await preflightChecks(benchmarkRoot, {
			live,
			blenderExecutable: parsed.flags.get("--blender") as string | undefined,
			addonPath: parsed.flags.get("--addon") as string | undefined,
		});
		const failures = checks.filter((check) => !check.ok);
		print({
			ok: failures.length === 0,
			mode: live ? "live-four-run" : "offline-check",
			checks,
	...
	if (command === "simulate") {
		...
		const scenarios = stringFlag(
			parsed,
			"--scenarios",
			join(benchmarkRoot, "fixtures", "calibration", "synthetic"),
		);
		const result = await runSweep({
			benchmarkRoot,
			planPath: stringFlag(parsed, "--plan"),
			runsRoot: stringFlag(parsed, "--runs"),
			selectedCells: listFlag(parsed, "--cells"),
			adapter: new SyntheticAgentAdapter(scenarios),
			blenderExecutable: parsed.flags.get("--blender") as string | undefined,
		});
	...
	if (command === "grade") {
		...
		const ids = parsed.flags.has("--run-id")
		...
		for (const id of ids) {
			...
			grades.push(
				await gradeRun({
					benchmarkRoot,
					runRoot: join(runs, id),
					runId: id,
					task,
					arm: record.arm,
					blenderExecutable: parsed.flags.get("--blender") as
						| string
						| undefined,
					fixtureHashBefore: record.inputs.fixture_sha256_before,
				}),
			);
		}
		print({
			...
			outcomes: grades.map((grade) => `${grade.run_id}:${grade.status}`),
		});
		return 0;
	}
	...
	if (command === "visual") {
		...
		if (subcommand === "bundle") {
			const result = await createBlindBundles({
				runsRoot: stringFlag(parsed, "--runs"),
				outputRoot: stringFlag(parsed, "--output"),
				seed: numberFlag(parsed, "--seed", 9001),
				duplicateFraction: config.visual.duplicate_fraction,
				minimumRaters: config.visual.minimum_raters,
				benchmarkRoot,
				blenderExecutable: stringFlag(
					parsed,
					"--blender",
					process.env.BLENDER_EXECUTABLE ??
						"/Applications/Blender.app/Contents/MacOS/Blender",
			...
			print({
				ok: true,
				blinded_items: result.items.length,
				forms: result.forms,
			});
			return 0;
		}
		if (subcommand === "close") {
			...
			print({
				ok: true,
				scored_items: Object.keys(result.aggregate).length,
				scored_runs: Object.keys(result.run_scores).length,
			});
			return 0;
		}
	...
	if (command === "report") {
		...
		print({
			...
			output: stringFlag(parsed, "--output"),
		});
		return 0;
	}
...
export async function runCli(): Promise<void> {
	try {
		...
		print({
			...
			help: usage
				? `Run ${BIN} --help`
				: "Inspect benchmark logs and configuration",
		});
