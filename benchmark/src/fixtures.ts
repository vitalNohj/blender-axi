...
import { readJson, sha256, sha256File, stableJson, writeJsonAtomic } from "./util.js";
...
export async function loadTasks(benchmarkRoot: string): Promise<TaskManifest[]> {
  const taskRoot = join(benchmarkRoot, "fixtures", "tasks");
  const names = (await readdir(taskRoot)).filter((name) => /^P[1-6]\.json$/u.test(name)).sort();
  return await Promise.all(names.map((name) => readJson<TaskManifest>(join(taskRoot, name))));
}

export async function fixtureManifestHash(benchmarkRoot: string): Promise<string> {
...
export async function createFixtures(
  ...
  for (const task of tasks) {
    ...
    if (!artifactName) throw new Error(`No fixture artifact mapping for ${task.id}`);
    ...
    if (task.id === "P5") {
      ...
      extras.push({ path: `generated/P5/faulty_build.py`, sha256: await sha256File(scriptPath) });
    }
    const artifactHash = await sha256File(artifactPath);
    if (task.fixture.immutable_sha256 !== "TO_BE_FROZEN" && task.fixture.immutable_sha256 !== semanticHash) {
      throw new Error(`${task.id} semantic fixture hash ${semanticHash} differs from frozen ${task.fixture.immutable_sha256}`);
    }
    entries.push({
      task_id: task.id,
  ...
  const index: FixtureIndex = { ...indexWithoutHash, manifest_sha256: sha256(stableJson(indexWithoutHash)) };
  await writeJsonAtomic(join(root, "fixtures", "manifest.json"), index);
  return index;
}

export async function verifyFixtures(benchmarkRoot: string): Promise<{ ok: boolean; errors: string[]; index: FixtureIndex }> {
  const root = resolve(benchmarkRoot);
  const index = await readJson<FixtureIndex>(join(root, "fixtures", "manifest.json"));
  const errors: string[] = [];
  ...
  if (index.manifest_sha256 !== expectedManifestHash) errors.push("fixture manifest hash mismatch");
  const tasks = new Map((await loadTasks(root)).map((task) => [task.id, task]));
  for (const entry of index.entries) {
    const artifact = join(root, "fixtures", entry.artifact_path.replace(/^generated\//u, "generated/"));
    try {
      if ((await sha256File(artifact)) !== entry.artifact_sha256) errors.push(`${entry.task_id} artifact hash mismatch`);
      const task = tasks.get(entry.task_id);
      if (!task || task.fixture.immutable_sha256 !== entry.semantic_sha256) errors.push(`${entry.task_id} task immutable fixture hash mismatch`);
      if ((await sha256File(join(root, entry.contract_path))) !== entry.contract_sha256) errors.push(`${entry.task_id} contract hash mismatch`);
      for (const extra of entry.extra_files) {
        const path = join(root, "fixtures", extra.path.replace(/^generated\//u, "generated/"));
        if ((await sha256File(path)) !== extra.sha256) errors.push(`${entry.task_id} extra file hash mismatch: ${extra.path}`);
      }
    } catch (error) {
      errors.push(`${entry.task_id} fixture missing: ${(error as Error).message}`);
    }
  }
  return { ok: errors.length === 0, errors, index };
}

export async function materializeReadOnlyFixture(benchmarkRoot: string, taskId: string, destination: string): Promise<string> {
  const verification = await verifyFixtures(benchmarkRoot);
  if (!verification.ok) throw new Error(`Fixture verification failed: ${verification.errors.join("; ")}`);
  const entry = verification.index.entries.find((candidate) => candidate.task_id === taskId);
  if (!entry) throw new Error(`Unknown task fixture: ${taskId}`);
  const sourceDirectory = dirname(join(benchmarkRoot, "fixtures", entry.artifact_path));
...
export async function fixtureBytes(benchmarkRoot: string, taskId: string): Promise<Uint8Array> {
  const index = await readJson<FixtureIndex>(join(benchmarkRoot, "fixtures", "manifest.json"));
