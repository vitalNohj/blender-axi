...
export async function createRunLayout(benchmarkRoot: string, runsRoot: string, runId: string, taskId: string): Promise<RunLayout> {
  ...
  for (const directory of [layout.workspace, layout.output, layout.logs, layout.transcript, layout.artifacts, layout.oracles]) {
...
export async function chooseUniquePort(minimum: number, maximum: number, seed: number): Promise<number> {
...
export async function assertPortClosed(port: number): Promise<void> {
  if (!(await listenable(port))) throw new Error(`Port ${port} is already in use`);
}

export async function registerOwnedProcess(path: string, process: OwnedProcess): Promise<void> {
  let current: OwnedProcess[];
  try {
    current = JSON.parse(await readFile(path, "utf8")) as OwnedProcess[];
  } catch (error) {
    throw new Error(`Cannot read owned-process registry ${path}: ${(error as Error).message}`, { cause: error });
  }
...
export async function cleanupOwnedProcesses(registryPath: string, timeoutMilliseconds = 15_000): Promise<void> {
  ...
  for (const processInfo of owned) if (isAlive(processInfo.pid)) process.kill(processInfo.pid, "SIGTERM");
  ...
  for (const processInfo of owned) if (processInfo.port !== null) await assertPortClosed(processInfo.port);
  await writeJsonAtomic(registryPath, []);
}

export function sanitizedEnvironment(layout: RunLayout, port: number, inherited: NodeJS.ProcessEnv = process.env): NodeJS.ProcessEnv {
  const allow = ["PATH", "TMPDIR"];
  const environment: NodeJS.ProcessEnv = {};
  for (const key of allow) if (inherited[key] !== undefined) environment[key] = inherited[key];
...
export async function writeBlenderStartup(path: string, port: number, addonPath: string): Promise<void> {
...
export async function spawnOwned(
  ...
  executable: string,
  args: string[],
  options: { cwd: string; env: NodeJS.ProcessEnv; port?: number; stdoutPath: string; stderrPath: string },
): Promise<ReturnType<typeof spawn>> {
  const stdout = await import("node:fs").then(({ openSync }) => openSync(options.stdoutPath, "a", 0o600));
  const stderr = await import("node:fs").then(({ openSync }) => openSync(options.stderrPath, "a", 0o600));
  const child = spawn(executable, args, { cwd: options.cwd, env: options.env, stdio: ["ignore", stdout, stderr], detached: false });
...
export async function removeRunLayout(runsRoot: string, runId: string): Promise<void> {
  const target = assertInside(resolve(runsRoot), join(resolve(runsRoot), runId));
  await chmod(join(target, "fixture"), 0o700).catch((error: NodeJS.ErrnoException) => {
