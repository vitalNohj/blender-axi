...
export async function generatePlan(benchmarkRoot: string, options: PlanOptions): Promise<SeedPlan> {
  const random = mulberry32(options.seed);
  const blocks: Array<{ taskId: string; replicate: number; sequence: "AXI_MCP" | "MCP_AXI"; seed: number }> = [];
  ...
  for (const block of shuffle(blocks, random)) {
    const arms: Arm[] = block.sequence === "AXI_MCP" ? ["axi", "mcp"] : ["mcp", "axi"];
...
export async function generatePreflightPlan(benchmarkRoot: string, seed = 4103): Promise<SeedPlan> {
  const random = mulberry32(seed);
  const p1Sequence: "AXI_MCP" | "MCP_AXI" = random() < 0.5 ? "AXI_MCP" : "MCP_AXI";
  const p5Sequence = p1Sequence === "AXI_MCP" ? "MCP_AXI" : "AXI_MCP";
  const cells: PlanCell[] = [];
  for (const [taskId, sequence] of [["P1", p1Sequence], ["P5", p5Sequence]] as const) {
    const pairSeed = Math.floor(random() * 2_147_483_647);
    const pairId = `${taskId}-preflight-${pairSeed}`;
    const arms: Arm[] = sequence === "AXI_MCP" ? ["axi", "mcp"] : ["mcp", "axi"];
    arms.forEach((arm, index) => cells.push({
      cell_id: `${pairId}-${arm}`,
      pair_id: pairId,
      task_id: taskId,
      replicate: 1,
