	...
	events.forEach((event, eventIndex) => {
		...
		if (arm === "axi") {
			if (event.interface === "mcp" || /^mcp__/iu.test(event.tool)) {
				violations.push({
					event_index: eventIndex,
					rule: "wrong-interface",
					detail: `AXI arm called MCP tool ${event.tool}`,
				});
			}
			const executable = shellExecutable(event);
			if (
				event.interface === "shell" &&
				executable !== null &&
				executable !== "blender-axi"
			) {
				violations.push({
					event_index: eventIndex,
					rule: "shell-allowlist",
					detail: `AXI shell executable ${executable} is not allowed`,
				});
			}
			if (
				/execute_blender_code|get_viewport_screenshot|blender-mcp|\buvx\b/iu.test(
					text,
				)
			) {
				violations.push({
					event_index: eventIndex,
					rule: "wrong-interface",
					detail: "AXI event references BlenderMCP surface",
				});
			}
		} else {
			if (event.interface === "shell") {
				violations.push({
					event_index: eventIndex,
					rule: "wrong-interface",
					detail: "MCP arm used a shell interface",
				});
			}
			if (
				event.interface === "mcp" &&
				!/^(mcp__)?blender(?:__|$)/iu.test(event.tool)
			) {
				violations.push({
					event_index: eventIndex,
					rule: "mcp-allowlist",
					detail: `MCP tool ${event.tool} is not on pinned Blender server`,
				});
			}
			if (/blender-axi|BLENDER_AXI_PORT/iu.test(text)) {
				violations.push({
					event_index: eventIndex,
					rule: "wrong-interface",
					detail: "MCP event references blender-axi",
				});
			}
		}
	});
...
export async function checkTranscriptPolicy(
	arm: Arm,
	path: string,
