...
export async function preflightChecks(
	...
	options: { live: boolean; blenderExecutable?: string; addonPath?: string },
): Promise<Check[]> {
	const config = await readJson<FrozenConfig>(
		join(benchmarkRoot, "config", "frozen.json"),
	);
	const agent = await readJson<AgentCommand>(
		join(benchmarkRoot, "config", "agent-command.json"),
	);
	const fixture = await verifyFixtures(benchmarkRoot).catch((error) => ({
		ok: false,
	...
	checks.push({
		id: "addon-pin",
		ok: addonHash === config.versions.addon_sha256,
		detail: addonHash
			? `addon sha256 ${addonHash}`
			: "BLENDER_MCP_ADDON_PATH unavailable",
	});
	checks.push({
		id: "unity-contract",
		ok:
			config.versions.unity_fixture_contract === "unity-urp-fbx-v1" &&
			config.versions.unity_pipeline === "URP",
		detail: "Unity URP/FBX-first fixture contract encoded",
	});
	checks.push({
		id: "limits",
		ok:
			config.limits.max_invalid_attempts !== null &&
			config.limits.max_critical_failures !== null,
		detail: "Safety invalid and critical ceilings configured",
	});
	const oracleFiles = [
		"inspect_scene.py",
		"roundtrip_import.py",
		"standardized_render.py",
	];
	const oracleChecks = await Promise.all(
		oracleFiles.map(async (name) => {
			try {
	...
	checks.push({
		id: "oracles",
		ok: oracleChecks.every(Boolean),
		detail:
			"Deterministic scene, exchange round-trip, and standardized render harnesses are readable",
	});
	for (const port of [9876])
		checks.push({
			id: `owned-port-${port}`,
			ok: !(await canConnect(port)),
			detail: (await canConnect(port))
				? `Port ${port} has a listener; preflight refuses to touch it`
				: `Port ${port} is closed`,
		});
	...
	if (options.live) {
		...
		for (const variable of agent.credential_environment_variables)
			checks.push({
				id: `credential-${variable}`,
				ok: Boolean(process.env[variable]),
				detail: `${variable} is required and value is not logged`,
			});
		checks.push({
			id: "unity-target",
			ok: Boolean(config.versions.unity_editor),
			detail:
				"Exact Unity editor patch required because preflight plan includes only P1/P5, but campaign readiness freezes all pins",
		});
	}
	return checks;
}
