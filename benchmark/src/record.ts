...
import type { AttemptRecord, PlanCell, TaskManifest, ToolEvent } from "./types.js";
...
export async function buildAttemptRecord(input: RecordInput): Promise<AttemptRecord> {
  ...
  const validityStatus = input.validity?.status ?? (policyViolation ? "invalid" : "valid");
  const invalidReason = input.validity?.invalid_reason ?? (policyViolation ? "wrong_interface_or_policy_violation" : null);
  return {
    ...
    arm: input.cell.arm,
    randomization: { seed: input.cell.seed, order_in_pair: input.cell.order_in_pair, sequence: input.cell.sequence },
    validity: {
      status: validityStatus,
      invalid_reason: invalidReason,
      replacement_for: input.validity?.replacement_for ?? input.cell.replacement_for,
    ...
    inputs: {
      ...
      oracle_manifest_sha256: sha256(stableJson(input.task.deterministic_oracles)),
      total_first_turn_context_bytes: Buffer.byteLength(firstTurn),
      total_first_turn_context_offline_tokens: tokenCount ? tokenCount(firstTurn) : null,
      marginal_interface_surface_bytes: Buffer.byteLength(input.interfaceSurface),
      marginal_interface_surface_offline_tokens: tokenCount ? tokenCount(input.interfaceSurface) : null,
    },
    cache: {
      regime: input.cell.cache_regime,
      provider_cache_key_or_observation: input.cache?.provider_cache_key_or_observation ?? null,
      creation_tokens: input.cache?.creation_tokens ?? null,
      read_tokens: input.cache?.read_tokens ?? null,
    },
    usage: {
      provider_input_tokens_total: input.usage?.provider_input_tokens_total ?? null,
      provider_input_tokens_uncached: input.usage?.provider_input_tokens_uncached ?? null,
      ...
      offline_tokens_total: tokenCount ? tokenCount(input.fullTranscript) : null,
      tool_argument_bytes: eventSummary.argument_bytes,
      tool_argument_offline_tokens: tokenCount ? tokenCount(input.events.map((event) => JSON.stringify(event.arguments)).join("\n")) : null,
      tool_response_bytes: eventSummary.response_bytes,
      tool_response_offline_tokens: tokenCount ? tokenCount(input.events.map((event) => JSON.stringify(event.response)).join("\n")) : null,
      api_cost_usd: input.usage?.api_cost_usd ?? null,
      pricing_source: input.usage?.pricing_source ?? null,
    },
    trajectory: {
      agent_turns: input.usage && "agent_turns" in input.usage ? Number(input.usage.agent_turns) : 0,
    ...
    outcome: {
      ...
      failure_stage: input.grade.functional_success ? null : "deterministic_grading",
    ...
    artifacts,
    oracles: { results: input.grade.oracle_results, visual_scores_path: null, unity_import_path: null },
    environment: {
      machine_id: machine,
...
export async function currentFixtureHash(runRoot: string, task: TaskManifest): Promise<string | null> {
  if (!task.fixture.source_artifact) return null;
  try {
    return await sha256File(join(runRoot, "fixture", task.fixture.source_artifact));
  } catch {
    return null;
  }
}
