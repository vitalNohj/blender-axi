...
export interface CheckDefinition {
  id: string;
  kind: "artifact" | "scene" | "preservation" | "reopen" | "gltf" | "fbx_roundtrip" | "unity" | "first_failure";
  required: boolean;
  points: number;
...
export interface AttemptRecord {
  ...
  artifacts: Array<{ kind: string; path: string; sha256: string; bytes: number; valid: boolean | null }>;
  oracles: { results: OracleResult[]; visual_scores_path: string | null; unity_import_path: string | null };
  environment: {
    machine_id: string;
    os: string;
    cpu: string;
