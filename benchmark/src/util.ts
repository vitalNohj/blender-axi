...
export async function readJson<T>(path: string): Promise<T> {
  ...
  try {
    ...
    throw new Error(`Invalid JSON at ${path}: ${(error as Error).message}`, { cause: error });
  }
}
...
export function assertInside(root: string, target: string): string {
  ...
  if (child === "" || (!child.startsWith(`..${sep}`) && child !== ".." && !child.startsWith(sep))) return resolvedTarget;
  throw new Error(`Path escapes benchmark root: ${target}`);
}

export function redact(value: unknown, secretValues: string[] = []): unknown {
  const sensitive = /(authorization|api[_-]?key|access[_-]?token|refresh[_-]?token|password|secret|cookie)/iu;
  const replace = (text: string): string => {
    let output = text
      .replace(/\b(sk-[A-Za-z0-9_-]{12,}|gh[pousr]_[A-Za-z0-9_]{20,}|Bearer\s+[A-Za-z0-9._~+/-]+=*)\b/giu, "[REDACTED]");
    for (const secret of secretValues.filter(Boolean)) output = output.split(secret).join("[REDACTED]");
    return output;
  };
...
export async function fileMetadata(path: string): Promise<{ sha256: string; bytes: number }> {
