...
describe("seed plans", () => {
  ...
  it("counterbalances and archives unique pilot cells", async () => {
    const plan = await generatePlan(root, { kind: "pilot", taskIds: ["P1", "P2"], replicates: 4, seed: 8 });
    expect(plan.cells).toHaveLength(16);
    expect(new Set(plan.cells.map((cell) => cell.cell_id)).size).toBe(16);
    for (const task of ["P1", "P2"]) {
      const sequences = plan.cells.filter((cell) => cell.task_id === task && cell.order_in_pair === 1).map((cell) => cell.sequence);
