...
describe("argument parsing", () => {
  it("parses positionals, value flags, equals flags, and booleans", () => {
    const parsed = parseArgs(["build.py", "--save", "/tmp/a.blend", "--render=front,side", "--json"], ["--save", "--render"]);
    expect(parsed.positionals).toEqual(["build.py"]);
    expect(Object.fromEntries(parsed.flags)).toEqual({ "--save": "/tmp/a.blend", "--render": "front,side", "--json": true });
  });
