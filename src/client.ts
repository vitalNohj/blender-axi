...
import { generatePrelude, parseExecutionOutput, type ExecutionResult, type PreludeOptions } from "./prelude.js";
...
class DeadPortError extends Error {
  constructor(
    ...
    super(`No Blender addon answered for session "${session}" on port ${port}`, { cause });
  }
}
...
export async function executeSource(
  ...
  if (!raw || typeof raw !== "object" || !("result" in raw) || typeof raw.result !== "string") {
    throw new Error("Malformed execute_code response from Blender addon");
  }
  return parseExecutionOutput(raw.result);
}

export function readPythonSource(file: string): { source: string; filename: string } {
  if (file === "-") return { source: readFileSync(0, "utf8"), filename: "<stdin>" };
