...
export async function launchBlender(context: SessionContext): Promise<number> {
  ...
  const child = spawn(process.env.BLENDER_AXI_BLENDER ?? DEFAULT_BLENDER, [
    "--python-expr",
    launchExpression(context.port),
  ], {
    detached: true,
...
export async function ensureBlender(context: SessionContext, launch: boolean): Promise<void> {
  try {
...
async function waitForBlender(context: SessionContext, timeoutMs = 30_000): Promise<void> {
  ...
  throw new Error(`Blender did not start session "${context.session}" on port ${context.port} within ${timeoutMs}ms`, { cause: lastError });
}
...
export function stopBlender(context: SessionContext): number | undefined {
  ...
  if (!Number.isSafeInteger(pid) || pid <= 0) throw new Error(`Invalid Blender PID file: ${pidFile}`);
  try {
