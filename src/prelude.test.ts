...
describe("prelude", () => {
  ...
  it("orders save before glb export and rendering", () => {
    ...
    expect(code.indexOf("save_as_mainfile")).toBeLessThan(code.indexOf("export_scene.gltf"));
    expect(code.indexOf("save_as_mainfile")).toBeLessThan(code.indexOf("bpy.ops.render.render"));
    expect(code).toContain("with redirect_stdout(io.StringIO())");
  });

  it("parses success and failure envelopes", () => {
    expect(parseExecutionOutput(`${RESULT_MARKER}{"ok":true,"stdout":"hi","artifacts":[]}\n`)).toEqual({ ok: true, stdout: "hi", artifacts: [] });
    expect(parseExecutionOutput(`${RESULT_MARKER}{"ok":false,"error":"bad","traceback":"Traceback","stdout_before_failure":"before"}\n`)).toMatchObject({ ok: false, traceback: "Traceback" });
  });
});
