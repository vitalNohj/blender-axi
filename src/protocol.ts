...
export function sendRequest(
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.once("timeout", () => finish(new Error(`Blender addon timed out after ${timeoutMs}ms`)));
    ...
    socket.once("close", () => {
      ...
      try {
        const parsed = tryParseResponse(response);
        finish(parsed ? undefined : new Error("Blender addon closed the socket before sending complete JSON"), parsed);
      } catch (error) {
        finish(error instanceof Error ? error : new Error(String(error)));
      }
    });
