...
export function validateSessionName(name: string): void {
	...
	if (!/^[A-Za-z0-9._-]{1,64}$/.test(name)) {
		throw new Error(
			`Invalid BLENDER_AXI_SESSION "${name}": use 1-64 chars from [A-Za-z0-9._-]`,
		);
	}
	if (/^\.+$/.test(name)) {
		throw new Error(
			`Invalid BLENDER_AXI_SESSION "${name}": a name made only of dots would collapse onto the default session's state directory`,
		);
	}
}
...
export function resolveSessionPort(
	...
	if (explicit !== undefined) {
		if (!/^\d+$/.test(explicit)) {
			throw new Error(
				`Invalid BLENDER_AXI_PORT "${explicit}": use an integer from 1 to 65535`,
			);
		}
